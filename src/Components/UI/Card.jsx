...
export const DisplayCard = ({ icon, title, quantity, subTitle }) => {
  return (
    <div className="w-52 h-full bg-card-gradient pl-8 pt-8 rounded-3xl">
...
export const GlowCard = ({ title, desctiption, value, percent, color }) => {
  ...
  return (
    <div
      className=" bg-[#282C35] flex w-96 h-40 rounded-3xl p-5 m-3 transition-all duration-300"
      style={{
        boxShadow: isHovered
    ...
    >
      ...
      <div className="w-1/2 h-full relative flex ">
        <div className="w-36 h-36 bg-gauge-gradient rounded-full p-3">
          <CircularProgressbar
            value={percent}
            text={`${percent}%`}
            strokeWidth={4}
